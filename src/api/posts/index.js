...
const authMiddleware = (req, res, next) => {
  ...
  try {
  const post = await postsModel.findOne({ _id: req.params.postId, user: req.user._id })
  if (post) {
  res.send(post)
  } else {
  ...
  try {
  const deletedPost = await postsModel.findOneAndDelete({ _id: req.params.postId, user: req.user._id })
  if (deletedPost) {
  res.status(204).send()
  } else {
  next(createHttpError(404, `Post with id ${req.params.postId} not found!`))
